    ...
    return (
      <div>
        <GridList cellHeight={120} className={classes.gridList} cols={4}>
          {props.enrollments.map((plan, i) => (
            <GridListTile key={i} className={classes.tile}>
              <Link to={"/learn/"+plan._id}><img className={classes.image} src={'/api/plans/photo/'+plan.plan._id} alt={plan.plan.name} /></Link>
              <GridListTileBar className={classes.tileBar}
                title={<Link to={"/learn/"+plan._id} className={classes.tileTitle}>{plan.plan.name}</Link>}
                actionIcon={<div className={classes.action}>
                 {plan.completed ? (<CompletedIcon color="secondary"/>)
